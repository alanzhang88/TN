...
NEWS_SIM_THRESHOLD = config.NEWS_SIM_THRESHOLD

def text_process(doc):
    nopuc = [c for c in doc if c not in string.punctuation]
    nopuc = ''.join(nopuc)
    words = [word for word in nopuc.split() if word.lower() not in stopwords.words('english')]
    stemmer = PorterStemmer()
    stems = [stemmer.stem(word) for word in words]
    return stems

def check_dup(query,docs):
    docs.insert(0,query)
    tfidf = TfidfVectorizer(docs,analyzer=text_process)
    docs_tfidf = tfidf.fit_transform(docs)
    #use linear since TfidfVectorizer will generate normalized vectors
    cos_sim = linear_kernel(docs_tfidf)
    print(cos_sim)
    for row in range(1,cos_sim.shape[0]):
        if cos_sim[row][0] > NEWS_SIM_THRESHOLD:
            print('Found Duplicate News')
            return True
    return False
...
if __name__ == "__main__":
    ...
    documents = [doc2,doc3,doc4]
    query = doc1
    check_dup(query,documents)
